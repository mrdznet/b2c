<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:46:"template/adminblue\Promotion\groupBuyList.html";i:1523873163;s:28:"template/adminblue\base.html";i:1522831681;s:45:"template/adminblue\controlCommonVariable.html";i:1522486340;s:32:"template/adminblue\urlModel.html";i:1510819828;s:34:"template/adminblue\pageCommon.html";i:1512444925;s:34:"template/adminblue\openDialog.html";i:1522669943;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<table class="mytable">
	<tr>
		<th width="20%" style="text-align:left;">
			<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('ADMIN_MAIN/promotion/addGroupBuy'); ?>';" style="margin:0 5px 0 0 !important;">添加团购活动</button>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/promotion/groupBuyList'); ?>",
		data : {
			...
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					if(data["data"][i]["goods_name"] != null && data["data"][i]["goods_name"] !=""){
						html += '<td class="align_left goods-name"><a href="'+__URL('SHOP_MAIN/goods/goodsinfo?goodsid='+data["data"][i]["goods_id"])+'" target="_blank">' + data["data"][i]["goods_name"] + '</a></td>';
					}else{
						html += '<td class="align_left goods-name"><a href="'+__URL('SHOP_MAIN/goods/goodsinfo?goodsid='+data["data"][i]["goods_id"])+'" target="_blank">-</a></td>';
					}
					...
					html += '<td>';
						html += '<a href="'+__URL('ADMIN_MAIN/Promotion/updateGroupBuy?group_id=' + data["data"][i]["group_id"])+'">编辑</a>&nbsp;';
	...
	$("#dialog").dialog({
		buttons: {
			"确定": function() {
				$.ajax({
					type:"post",
					url : "<?php echo __URL('ADMIN_MAIN/promotion/delGroupBuy'); ?>",
					data : { "group_id" : group_id.toString()},
					dataType : "json",
					success : function(data) {
						if (data["code"] > 0) {
							showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/promotion/groupbuylist'); ?>");
						}else{
