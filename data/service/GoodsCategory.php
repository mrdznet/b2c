    ...
    public function getGoodsCategoryBlockQuery($shop_id, $show_num = 4)
    {
        ...
        if (empty($cache)) {
            ...
            $goods_category_block_list = $goods_category_block->getQuery([
                "shop_id" => $shop_id,
                "is_show" => 1
            ], "color,ad_picture,category_alias,category_id,goods_sort_type", "sort asc");
            foreach ($goods_category_block_list as $k => $v) {
                $order = "ng.sort asc,ng.create_time desc";
                switch ($v["goods_sort_type"]) {
                    case 0:
...
$goods_list = $goods->getGoodsQueryLimit($condition, "ng.goods_id,ng_sap.pic_cover_small,ng.goods_name,ng.price,ng.introduction,ng.promotion_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.point_exchange_type,ng.point_exchange", $show_num, $order);
